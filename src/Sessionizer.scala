...
import com.software.sparkstreaming.Utilities.{apacheLogPattern, setupLogging}
...
object Sessionizer {
  ...
  /** This "case class"  lets  us quickly define a complex type that contains a session length and a list of
   * URL's visited, which makes up the session data state that we want to preserve across a given session.
   * The "case class" automatically creates the constructor and accessors we want to use.*/
  case class SessionData(val sessionLength: Long, val clickstream: List[String]);

  /** This function gets called as new data is streamed in, and maintains state across whatever key you define. In this
   * case, it expects to get an IP address as the key, a String URL (wrapped in an Option to handle exceptions), and
   ...
   */
  def trackStateFunc(batchTime: Time, ip: String, url: Option[String], state: State[SessionData]):
    ...
    // Create a new state that increments the session length by one, adds this URL to the clickstream, and clamps the
    // clamps the clickstream list to 10 items
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Create a socket stream to read log data published via netcal on port 9999 locally
    ...
    // Now we will precess this data through our StateSpec to update the stateful session data
    ...
    val spark = SparkSession
      ...
      // Slightly different syntax here from out earlier SparkSQL example. toDF can take a list
      ...
      val requestsDataFrame = rdd.map(x => (x._1, x._2.sessionLength, x._2.clickstream)).toDF("ip",
                                            "sessionLength", "clickstream")
      // Create a SQL table from this DataFrame
      requestsDataFrame.createOrReplaceTempView("sessionData")

      // Dump out the results - you can do any SQL you want here.
      val sessionDataFrame =
