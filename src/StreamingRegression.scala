...
import com.software.sparkstreaming.Utilities.setupLogging
...
// Example of using streaming linear regression with stochastic gradient descent (SGD)
object StreamingRegression {
  ...
  def main (args: Array[String]): Unit = {
    ...
    // and y is the "feature" which is some associated value we cal use to predict the label by
    ...
    // This will expect both the know label and the feature data. In the real world you won't know
    // the "correct" value and would just input feature data, byt this lets us validate out results.
    ...
    // Now we will build up out linear regression model as new training data is received.
    val numFeatures = 1
    ...
    // And as test data is received, we'll tun vur model on it to predict values based on our linear
